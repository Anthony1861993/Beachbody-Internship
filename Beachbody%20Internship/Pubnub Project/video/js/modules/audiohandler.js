define(['jquery','menu','cookie'], function($,menu,cookie){
	...
	audiohandler.prototype.collectFlashTrackData = function(event) {
		...
		console.log("[BB]["+moduleData.module_designator+"] moduleData.data.audio_tracks: ", moduleData.data.audio_tracks_raw);
		//console.log("[BB]["+moduleData.module_designator+"] moduleData.data.image_tracks: ", moduleData.data.availableImageTracks);
	...
	audiohandler.prototype.menuItemSelectHandler = function(audioTrackIndex, bypassCookie) {
		...
		if (audioTrackIndex > 0) {
			...
			if (!bypassCookie) {
				cookie.setCookie(bbPlayer.config.data.mpxReferenceID+"_audio", pdkAdjustedIndex, INTERVAL_2_DAY_MS); // Set cookie here. Prepends program name so that these settings will only persist within the program.
			}
		}
	}
	...
	audiohandler.prototype.selectAudioTrackByIndex = function(audioTrackIndex) {
		...
		// Update the global audio track object.
		globalData.currentAudioTrack = self.constructAudioTrackObjectWithData(moduleData.data.active_tracks.current);
	}
	...
	audiohandler.prototype.constructAudioSwitchObject = function(){
		...
		if (moduleData.data.active_tracks.previous) {
			oldAudioTrackData = self.constructAudioTrackObjectWithData(moduleData.data.active_tracks.previous);
		}

		if (moduleData.data.active_tracks.current) {
			newAudioTrackData = self.constructAudioTrackObjectWithData(moduleData.data.active_tracks.current);
		}
	...
	// Constructs a single audio track object using the passed data.
	audiohandler.prototype.constructAudioTrackObjectWithData = function(audioTrackData){
		var newAudioTrack = null;

		newAudioTrack = {globalDataType:"com.theplatform.com.data::AudioTrack", index:audioTrackData.index, language:audioTrackData.data.language, title:audioTrackData.data.markup};
